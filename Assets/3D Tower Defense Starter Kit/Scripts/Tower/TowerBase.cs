...
public class TowerBase : MonoBehaviour
{
    ...
    //effect to instantiate on shoot
    ...
    void Start()
    {
        ...
        if (turret)
        {
            ...
            if (turScript)
                ...
                Debug.LogWarning("Turret set on '" + gameObject.name + "but found no TowerRotation script attached to it!");
        }
            ...
            case ShootOrder.Weakest:
                ...
                switch (shootOrder)
                {
                    ...
                    case ShootOrder.Weakest:
                        ...
                        {
                            //initialize temporary health value
                            //(devised value, a possible enemy should have less than 999999 health)
                            float lowestHealth = 999999;
    ...
    void ShotEffect()
    {
        ...
        if (shotEffect)
        {
            //get enemy direction relative to shot position
            Vector3 relativePos = currentTarget.position - shotPos.position;
            //create rotation so shootEffect will look in that direction
            Quaternion rotation = Quaternion.LookRotation(relativePos);
            //instantiate particle effect
            PoolManager.Pools["Particles"].Spawn(shotEffect, shotPos.position, rotation);
        }
         */
    }
    ...
    void InstantiateProjectile(Vector3 targetPos)
    {
        ...
        proj.damage = proj.damage * damageMultiplier;
    }
