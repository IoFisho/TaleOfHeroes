...
public class RangeTrigger : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider col)
    {
        ...
        //skip enemy if it died already
        if (!colGO.activeInHierarchy || PoolManager.Props[colGO.name].health <= 0)
            return;

        //here we check against attackable enemyTypes:
        //if collided object is in layer "Enemies" and whether we can attack air and ground targets
        //in case it is an potential enemy and its tag is in line with our possible targets, continue
        if (colGO.layer == SV.enemyLayer && (towerScript.myTargets == TowerBase.EnemyType.Both
