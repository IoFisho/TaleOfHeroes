...
public class SelfControl : MonoBehaviour
{
    ...
    //tower shoot delay value (animates reload texture)
                           ...
                           float towerHeight, bool mobile)
    {
        ...
        towerScript.StopAllCoroutines();
        towerScript.enabled = false;

        //set current radius and delay values through TowerBase.cs -> Upgrade.cs -> radius
        //(we cache them, so we don't need to call this every time in the functions below)
        int curLvl = towerScript.upgrade.curLvl;
        radius = towerScript.upgrade.options[curLvl].radius;
        shootDelay = towerScript.upgrade.options[curLvl].shootDelay;
        ...
        //we like to move our crosshair not until iTween's animation is completed and the camera reached its destination
    ...
    IEnumerator CoroutineUpdate()
    {
        ...
        while (true)
        {
            ...
            if (camDesktop)
            {
                ...
                if (Input.GetMouseButtonUp(0) && !UICamera.hoveredObject)
                {
                    Attack();
                }
            }
    ...
    void PositionCrosshair()
    {
        ...
        //UICamera.hoveredObject does not work on mobiles. Instead, on mobile
        ...
        if (UICamera.hoveredObject &&!centerCrosshair || SV.showExit)
            return;
    ...
    public void Attack()
    {
        //don't do anything if the delay time isn't up
        if (Time.time < shootDelay + towerScript.lastShot)
            return;
    ...
    public void Terminate()
    {
        ...
        //enable tower AI and disable range indicator
        ...
        float invokeInSec = shootDelay + lastShot - Time.time;
