...
public class GameInfo : MonoBehaviour
{
    ...
    public UILabel lbl_wave;
    //enemies alive count label
    public UILabel lbl_alive;
    //enemies killed count label
    public UILabel lbl_killed;
    //gold (health) count label
    public UILabel lbl_gold;
    //time until next wave starts label
    public UILabel lbl_waveTimer;
    //resources count label
    public UILabel[] lbl_resources;
    ...
    //initialize NGUI buttons
    void InitNGUI()
    {
        //NGUI - hook up method NextWave() to launch on start button click
        UIEventListener.Get(btn_startWave).onClick += NextWave;
    }
    ...
    private bool CheckWave()
    {
        ...
        switch (waveScript.waveStartOption)
        {
            ...
            //if the bool 'userInput' of our WaveManager is false or the wave index exceeds the wave count
            ...
            case WaveManager.WaveStartOption.userInput:
                if (!waveScript.userInput || GameHandler.wave >= GameHandler.waveCount)
                    return false;
                break;
            ...
            //this means that the current wave still has enemies alive, or the wave index exceeds wave count
            case WaveManager.WaveStartOption.roundBased:
                if (waveScript.IsInvoking("CheckStatus") || GameHandler.wave >= GameHandler.waveCount)
                    return false;
                break;
        }
    ...
    private void NextWave(GameObject button)
    {
