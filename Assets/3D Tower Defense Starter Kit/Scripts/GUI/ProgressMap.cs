...
public class ProgressMap : MonoBehaviour
{
    ...
    public static void AddToMap(GameObject mapObjectPrefab, int objID)
    {
        ...
        GameObject newObj = PoolManager.Pools["PM_StartingPoint"].Spawn(mapObjectPrefab,
                            PoolManager.Pools["PM_StartingPoint"].transform.position, Quaternion.identity);
        //resize it accordingly,
        //so NGUI automatically takes the screen size in account
        ...
        //set slider's size to fit the map size
        newMapObj.slider.fullSize = ProgressMap.instance.mapSize;
    ...
    internal IEnumerator Remove(int objID)
    {
        ...
        pMapObj.sprite.spriteName = pMapObj.objDeadSprite;
